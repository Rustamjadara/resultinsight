...
export default function UploadPage() {
  ...
  const { setParseResult, setLoading, students, school_name, class_level, stats } = useAppStore();
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const { students: parsed, school_name: sn, class_level: cl, warnings } = await parseRes.json();
      ...
      setParseResult(processed, sn, cl ?? "X", schoolStats, warnings);
      if (warnings.length) toast(`⚠ ${warnings.length} warning(s)`, { icon:"⚠️" });
      toast.success(`${processed.length} students loaded (Class ${cl ?? "X"})`);
    } catch (e) {
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              ...
              Class level (X or XII) is auto-detected from the file.
            </p>
          </div>
          {stats && (
        ...
        >
          ...
          {processing ? (
            ...
            <>
              ...
              <p className="text-gray-500 text-sm mt-1">or click to browse · Class X &amp; XII supported</p>
            </>
          )}
        </div>
                      ...
                      <tr>
                        ...
                        {allCodes.map(c => (
                          <th key={c} className="px-2 py-2 whitespace-nowrap">
                            {getSubjectName(c, class_level)}<br/>
              ...
              <button
                onClick={async () => {
                  ...
                  const res = await fetch("/api/export", {
                    method:"POST",
                    headers:{"Content-Type":"application/json"},
                    body: JSON.stringify({ students, school_name, class_level, stats, type:"analysis" }),
                  });
                  toast.dismiss("xl");
                  ...
                  a.href     = url;
                  a.download = `result_analysis_class_${class_level}.xlsx`;
                  a.click();
