...
import { getSubjectName, BAND_DEFS } from "./constants";

const MATH_STD  = new Set(["041"]);
const MATH_BAS  = new Set(["241"]);
const SCI_CODE  = "086";

/** Detect whether the gazette is for Class X or Class XII.
 *  Looks for explicit class markers in the header lines. */
function detectClassLevel(lines: string[]): "X" | "XII" {
  for (const line of lines.slice(0, 40)) {
    const upper = line.toUpperCase();
    // Common patterns: "CLASS : XII", "CLASS-XII", "CLASS XII", "STD XII", "SECONDARY (XII)"
    if (/\bCLASS\s*[-:]\s*XII\b/.test(upper)) return "XII";
    if (/\bCLASS\s+XII\b/.test(upper))         return "XII";
    if (/\bSTD\.?\s*XII\b/.test(upper))         return "XII";
    if (/\bXII\s+(BOARD|EXAM|RESULT|SCHOOL)\b/.test(upper)) return "XII";
    if (/SR\.?\s*SEC(ONDARY)?\b/.test(upper))   return "XII";
    if (/SENIOR\s+SEC(ONDARY)?\b/.test(upper))  return "XII";
    // Class X markers
    if (/\bCLASS\s*[-:]\s*X\b/.test(upper))    return "X";
    if (/\bCLASS\s+X\b/.test(upper))            return "X";
    if (/\bSTD\.?\s*X\b/.test(upper))           return "X";
    if (/\bSECONDARY\s+SCHOOL\b/.test(upper))   return "X";
    if (/\bSECSCHOOL\b/.test(upper))            return "X";
  }
  // Heuristic fallback: XII-only codes present → XII
  const allText = lines.join(" ");
  const xiiOnlyCodes = ["042", "043", "044", "054", "055", "301"];
  for (const code of xiiOnlyCodes) {
    if (new RegExp(`\\b${code}\\b`).test(allText)) return "XII";
  }
  return "X"; // default
}
...
export function parseGazette(text: string): {
  ...
  const class_level = detectClassLevel(lines);
  ...
  return { students, school_name, class_level, warnings };
}

export function computeSchoolStats(students: Student[]): SchoolStats {
  const classLevel = students[0]?.class_level ?? "X";
  ...
  for (const code of allCodes) {
    ...
    subjects[code] = {
      code, name: getSubjectName(code, classLevel),
