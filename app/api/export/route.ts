...
async function handleExcel(body: {
  ...
  const { type, students, school_name, class_level = "X", stats } = body;
  ...
  if (type === "analysis" && stats) {
    buildAnalysisWorkbook(wb, students, stats, school_name, class_level, date);
  } else {
    buildAllocationWorkbook(wb, students, school_name, date);
  }

  const buffer = await wb.xlsx.writeBuffer();
  const fname  = type === "analysis"
    ? `result_analysis_class_${class_level}`
    : "stream_allocation";
...
function buildAnalysisWorkbook(
  wb: any, students: Student[], stats: SchoolStats, school: string, classLevel: "X" | "XII", date: string,
) {
  ...
  t1.value     = `Result Analysis – Class ${classLevel} – ${school}  (${date})`;
